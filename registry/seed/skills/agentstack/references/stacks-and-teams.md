...
solo, to a team, or across a company. Building the stack definition and sharing
it need a registry token and publisher/admin role; installing the result does
not. Treat every mutation as shared state and confirm before running it.
...
`agentstack stack remove <org>/<stack> <org>/<skill>`.
