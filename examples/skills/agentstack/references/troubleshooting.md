...
A missing token is not a first-run failure. Unconfigured user-level targets
warn only when `~/.claude/skills` or `~/.codex/skills` already exists, and
still do not print `next:`.
`config show` prints the resolved configuration when a path or override is in
doubt. Add `agentstack registry show` only when the work uses a registry.
